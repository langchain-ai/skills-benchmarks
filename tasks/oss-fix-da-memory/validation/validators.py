...
def _run_test_script(test_dir: Path, module_path: Path) -> dict:
    """Run the test_memory.py script against the agent system.

    Returns the parsed JSON results.
    """
    test_script = Path(__file__).parent / "test_memory.py"
...
def validate_execution(test_dir: Path, outputs: dict) -> tuple[list[str], list[str]]:
    """Run pattern tests against the fixed agent system code.
    ...
    module_path = test_dir / "agent_system.py"
    if not module_path.exists():
        failed.append(f"Agent system file not found: {module_path}")
