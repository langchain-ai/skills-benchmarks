 ...
 * Test adding LangSmith tracing to this app.
 */
...
async function extractEntities(message: string): Promise<Entities> {
  /**
...
function lookupOrder(orderId: string): OrderInfo | null {
  /**
   * Mock order lookup.
   */
...
async function handleSupportRequest(message: string): Promise<string> {
  /**
  ...
  if (entities.order_id) {
    orderInfo = lookupOrder(entities.order_id);
  }
