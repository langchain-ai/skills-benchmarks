"""Test script for lc-framework-hybrid-pipeline validation.

Checks that Claude uses create_deep_agent as the orchestrator and registers
a LangGraph sub-workflow via CompiledSubAgent.
"""
...
def _uses_hybrid_deep_langgraph(content, label):
    p, f = [], []
    if "create_deep_agent" in content:
        p.append(f"{label}: correctly uses create_deep_agent as orchestrator")
    else:
        f.append(f"{label}: missing create_deep_agent (Deep Agents should be the orchestrator)")
    if "CompiledSubAgent" in content:
        p.append(f"{label}: correctly uses CompiledSubAgent to register LangGraph workflow")
    else:
        f.append(f"{label}: missing CompiledSubAgent (required to wire LangGraph graph into deep agent)")
    return p, f


def check_hybrid_pipeline(runner: TestRunner):
    """Hybrid Pipeline uses create_deep_agent + CompiledSubAgent."""
    check_file(
        runner.artifacts[0],
        "Hybrid Pipeline",
        [_uses_hybrid_deep_langgraph, _static_imports],
        runner,
    )
...
if __name__ == "__main__":
    TestRunner.run([check_hybrid_pipeline, check_outputs_metadata])
