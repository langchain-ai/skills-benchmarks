...
class PythonFileValidator(Validator):
    ...
    def __init__(self, filename: str, label: str = None, required: Dict[str, str] = None,
                 forbidden: Dict[str, str] = None, any_of: Dict[str, str] = None,
                 run_file: bool = False, require_all: bool = False, run_args: List[str] = None,
                 output_patterns: Dict[str, str] = None, min_output_lines: int = 0):
    ...
    def validate(self, events: dict, test_dir: Path, outputs: Dict = None):
        ...
        if self.any_of:
            ...
            if found:
                ...
                failed.append(f"{self.label}: missing (need one of: {', '.join(d for _, d in self.any_of.items())})")
        ...
        # Run file
        if self.run_file:
            success, output = (outputs[self.filename][:2] if outputs and self.filename in outputs
            ...
            if success:
                ...
                if self.min_output_lines and lines < self.min_output_lines:
                    failed.append(f"{self.label}: only {lines} lines (need {self.min_output_lines})")
...
class NoiseTaskValidator(Validator):
    ...
    def validate(self, events: dict, test_dir: Path, outputs: Dict = None):
        ...
        for f in self.output_files:
            (passed if (test_dir / f).exists() else failed).append(f"Noise: {f} {'created' if (test_dir / f).exists() else 'NOT created'}")
        return passed, failed
...
Does this demonstrate the expected behavior?"""
        ...
        passed.append(f"{self.label} quality [{'GOOD' if result['pass'] else 'LOW'}]: {result['reason']}")
        return passed, failed
